...
export function streak(sessions:Session[]){const days=new Set(sessions.map(s=>new Date(s.date).toLocaleDateString('en-CA')));let n=0;const d=new Date();if(!days.has(d.toLocaleDateString('en-CA')))d.setDate(d.getDate()-1);while(days.has(d.toLocaleDateString('en-CA'))){n++;d.setDate(d.getDate()-1);}return n;}
