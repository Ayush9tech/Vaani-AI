export async function coachRequest(task:string,payload:Record<string,unknown>){
 ...
 if(!response.ok)throw new Error('Coaching service unavailable');return response.json() as Promise<{result?:unknown;questions?:unknown[];source?:string;sessions?:unknown[];saved?:boolean;deleted?:boolean;notice?:string}>;
}
